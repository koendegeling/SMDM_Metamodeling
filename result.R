...
set.seed(123)
pars_training_lhs_norm_GP  <- maximinLHS(n = n_training_GP,  k = 4)
pars_training_lhs_norm_GAM <- maximinLHS(n = n_training_GAM, k = 4)

set.seed(456)
pars_testing_lhs_norm  <- maximinLHS(n = n_testing, k = 4)

# Providing names to the generated matrices
colnames(pars_training_lhs_norm_GP) <- colnames(pars_training_lhs_norm_GAM) <- colnames(pars_testing_lhs_norm) <- c('StartAge', 'Interval', 'NrScr', 'FITcutoff')

# Transforming the values on 0-1 scale to the original parameter scale
pars_training_lhs_org_GP  <- t(apply(pars_training_lhs_norm_GP,  1, fun_denormalize))
pars_training_lhs_org_GAM <- t(apply(pars_training_lhs_norm_GAM, 1, fun_denormalize))
...
out_training_GP  <- t(apply(pars_training_lhs_org_GP,  1, runHEM))
out_training_GAM <- t(apply(pars_training_lhs_org_GAM, 1, runHEM))
...
mm_gam <- gam(formula = NMB ~ s(StartAge) + s(Interval) + s(NrScr) + FITcutoff +  
              ...
              data = as.data.frame(out_training_GAM))
...
mm_gp <- GP_fit(X = pars_training_lhs_norm_GP, Y = out_training_GP[, 'NMB'])
...
par(mfrow = c(1, 2))        # plot three plots side-by-side
...
# Obtain the outcomes
out_experiments_GP  <- predict.GP(object = mm_gp, xnew = m_experiments_norm)$Y_hat
out_experiments_GAM <- predict.Gam(object = mm_gam, newdata = as.data.frame(m_experiments_org))

# Plot the NMB as the function of the StartAge
par(mfrow = c(1, 1))
plot(x = m_experiments_org[ , 'StartAge'], y = out_experiments_GAM, type = 'l',
     las = 1, xlab = 'StartAge', ylab = 'NMB (EUR 20k per QALY)',
     main = 'NMB as function of StartAge\nInterval = 2, FITcutoff = 75')
lines(x = m_experiments_org[ , 'StartAge'], y = out_experiments_GP, col = 'red')
legend('right', bty = 'n', col = c('black', 'red'), lty = 1, legend = c('GAM', 'GP'))
...
fn_optim_GP  <- function(x) predict.GP(object = mm_gp, xnew = matrix(data = x, ncol = 4))$Y_hat
fn_optim_GAM <- function(x) predict.Gam(object = mm_gam, newdata = data.frame(
  StartAge  = fun_denormalize(x)[1],
  Interval  = fun_denormalize(x)[2],
  NrScr     = fun_denormalize(x)[3],
  FITcutoff = fun_denormalize(x)[4]
))
...
c(
  value_GP  = optim_out_GP$value,
  value_GAM = optim_out_GAM$value
)

# Optimal screening strategy on the original parameter scale
rbind(
  pars_GP  = fun_denormalize(optim_out_GP$par),
  pard_GAM = fun_denormalize(optim_out_GAM$par)
)

# Evaluating the "optimal strategies" with the simulator, i.e. original HEM
# - Both overestimate the true NMB according to the simulator, but the GAM solution is better
rbind(
  GP  = runHEM(pars = fun_denormalize(optim_out_GP$par)),
  GAM = runHEM(pars = fun_denormalize(optim_out_GAM$par))
)
